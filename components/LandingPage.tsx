...
export default function LandingPage() {
  ...
  const [classroomChannel, setClassroomChannel] = useState('');
  ...
  const handleStartConversation = async () => {
    ...
    const requestedChannel = classroomChannel.trim();
    const isCreatingClassroom = !requestedChannel;

    try {
      // 1. Fetch RTC token + channel
      // console.log('Fetching Agora token...');
      const channelParam = requestedChannel
  ? `?channel=${encodeURIComponent(requestedChannel)}`
  : '';

const agoraResponse = await fetch(
  `/api/generate-agora-token${channelParam}`,
);
      ...
      const [agentData, rtm] = await Promise.all([
        // 2a. Start the AI agent only when creating a new classroom (not joining an existing one).
        // Student joining an existing classroom must not create another agent.
        isCreatingClassroom
      ...
      setRtmClient(rtm);
      console.log('CLASSROOM CHANNEL:', responseData.channel);
      setClassroomChannel(responseData.channel);
      setAgoraData({
      ...responseData,
        ...
        >
          {!showConversation ? (
            <QuickstartPreCallCard
              ...
              onClassroomChannelChange={setClassroomChannel}
              role={role}
